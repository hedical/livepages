// Analyse Acoustique — clone d'analyse-geotechnique.js (brique qualité AnalyticEvent,
// même log d'events, filtré sur les events Acousti* au lieu de Geotech).
// Configuration
...
// Data URL fetched from the webhook after authentication (n8n must expose ANALYSE_ACOUSTIQUE_URL).
let DATA_URL = '';
...
// Parse JSON output de la card Metabase Analyse Acoustique — one row per event (Notice or Report).
function parseAcoustiqueJSON(jsonArray) {
    if (!Array.isArray(jsonArray)) {
        console.warn('Invalid JSON array for acoustique');
        return [];
    }
    ...
    console.log('Parsed', data.length, 'acoustique events');
    return data;
}
...
function parseAcoustiqueCSV(csvString) {
    ...
    return parseAcoustiqueJSON(rows);
}
...
// Returns the ANALYSE_ACOUSTIQUE_URL via the webhook.
...
async function authenticateAndGetURL() {
    ...
    const tryParse = (text) => {
        const m = text.match(/ANALYSE_ACOUSTIQUE_URL\s*=\s*['"]([^'"]+)['"]/);
        return m ? m[1] : null;
    };
...
async function init() {
    try {
        ...
        if (!DATA_URL) {
            ...
            errorMessageEl.innerHTML = `Aucune URL <code>ANALYSE_ACOUSTIQUE_URL</code> n'est exposée par le webhook. Ajoute la ligne <code>ANALYSE_ACOUSTIQUE_URL = '&lt;supabase-url&gt;'</code> à la réponse n8n (cf. <a href="metabase-queries.md" class="underline">metabase-queries.md</a>, card Analyse Acoustique).`;
            return;
        }
        ...
        if (payload === null) {
            allData = parseAcoustiqueCSV(rawText);
            ...
            allData = Array.isArray(innerJson) ? parseAcoustiqueJSON(innerJson) : parseAcoustiqueCSV(inner);
            ...
            allData = parseAcoustiqueJSON(payload);
        }
        console.log('Loaded', allData.length, 'acoustique events');
