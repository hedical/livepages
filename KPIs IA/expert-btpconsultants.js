...
let EXPERT_DATA_URL = 'https://qzgtxehqogkgsujclijk.supabase.co/storage/v1/object/public/DataFromMetabase/expert_btpconsultants_ct.json';
...
function loadParameters() {
    const saved = localStorage.getItem('expert_btpconsultants_parameters');
    if (saved) {
        try {
...
function saveParameters() {
    localStorage.setItem('expert_btpconsultants_parameters', JSON.stringify(parameters));
}
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (Object.keys(population).length > 0) {
                console.log('Population loaded from Supabase:', Object.keys(population).length, 'agencies');
...
async function init() {
    ...
    if (!dataUrls) return;
    if (dataUrls.EXPERT_BTP_URL) EXPERT_DATA_URL = dataUrls.EXPERT_BTP_URL;
    ...
    try {
        ...
        // Update DRs based on metadata.management or agencyToDR mapping
        allData.forEach(item => {
            if (!item.dr && item.agencyCode && agencyToDR[item.agencyCode]) {
                item.dr = agencyToDR[item.agencyCode];
            }
        });
