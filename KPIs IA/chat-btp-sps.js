...
let CHAT_DATA_URL = 'https://qzgtxehqogkgsujclijk.supabase.co/storage/v1/object/public/DataFromMetabase/chat_btp_sps.json';
...
function loadParameters() {
    const saved = localStorage.getItem('chat_btp_sps_parameters');
    if (saved) {
        try {
...
function saveParameters() {
    localStorage.setItem('chat_btp_sps_parameters', JSON.stringify(parameters));
}
...
async function init() {
    ...
    if (!dataUrls) return;
    if (dataUrls.CHAT_BTP_SPS_URL) CHAT_DATA_URL = dataUrls.CHAT_BTP_SPS_URL;
    ...
    try {
        ...
        const dataResponse = await fetch(CHAT_DATA_URL);
