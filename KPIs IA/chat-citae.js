...
let CHAT_DATA_URL = 'https://qzgtxehqogkgsujclijk.supabase.co/storage/v1/object/public/DataFromMetabase/chat_citae.json';
...
function loadParameters() {
    const saved = localStorage.getItem('chat_citae_parameters');
    if (saved) {
        try {
...
function saveParameters() {
    localStorage.setItem('chat_citae_parameters', JSON.stringify(parameters));
}
...
async function init() {
    ...
    if (!dataUrls) return;
    if (dataUrls.CHAT_CITAE_URL) CHAT_DATA_URL = dataUrls.CHAT_CITAE_URL;
