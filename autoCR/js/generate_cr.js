// generate_cr.js — Génère un CR .docx pour une agence ou DR (version navigateur)
// Dépend de : docx (global CDN), helpers.js
// logoData = { data: Uint8Array, type: 'png'|'jpeg' } ou null
...
const STATUTS = {
  ...
  "A réaliser": { label:"○  À réaliser",  color:GRAY2, bg:GRAY3 },
...
function buildContent(entity, moisLabel, logoData) {
  ...
  ch.push(
    ...
    p(r("DE RÉUNION",    {bold:true, size:56, color:BLUE})),
    sp(20),
  ...
  ch.push(tbl([WLBL,WVAL], [
    ...[ ["Date","À compléter"],["Lieu","À compléter"],
         ["Directeur d'Agence","À compléter"],
         ["Participants","DA · RS SPS · RS CTC · Chefs de mission"],
         ["Ordre du jour","Commerce · Opérations · Prévisionnels N+1 · Affaires LT · RH"],
  ...
  ch.push(...themeTitle("1.1  Prise de commandes — Résultats annuels", C.acc));
  ...
  ch.push(...dataTable(
    ["Service","S1 (Jan–Juin)","S2 (Juil–Déc)","Mois en cours","Total"],
    [
  ...
  ch.push(analyseBloc([
    "À compléter : commentaire sur les tendances du semestre (SPS / CTC, évolutions notables).",
    "À compléter : point de vigilance ou anomalie à signaler.",
    "→ À compléter : action ou objectif commercial clé.",
  ]));
  ch.push(...planActions([
    {action:"À compléter", qui:"À compléter", statut:"A réaliser"},
    {action:"À compléter", qui:"À compléter", statut:"A réaliser"},
  ], C.acc));
  ch.push(...sep());

  ch.push(...themeTitle("1.2  Devis & Taux de transformation", C.acc));
  ch.push(note("Données à extraire depuis Qlik avant la réunion."));
  ch.push(sp(80));
  ch.push(...dataTable(
    ...
    [
      ["Devis émis (cumul)","—","—","—"],
      ["Taux de réussite AO","—","—","—"],
      ["Taux de succès montant","—","—","—"],
      ["Devis en cours > 50 k€","—","—","—"],
    ],
    [3200,1360,1360,2386], {headerBg:C.acc}
  ));
  ch.push(analyseBloc(["À compléter : commentaire sur le taux de transformation.","→ À compléter : action prioritaire."]));
  ch.push(...planActions([{action:"À compléter", qui:"À compléter", statut:"A réaliser"}], C.acc));
  ch.push(PB());

  // ── 02 OPÉRATIONS ────────────────────────────────────────────────
  ch.push(...sectionBanner("02","OPÉRATIONS","",O));
  ch.push(...themeTitle("2.1  Production — Résultats annuels", O.acc));
  ...
  ch.push(analyseBloc([
    "À compléter : commentaire sur l'évolution de la production.",
    "→ À compléter : action prioritaire.",
  ]));
  ch.push(...planActions([
    {action:"À compléter", qui:"À compléter", statut:"En cours"},
    {action:"À compléter", qui:"À compléter", statut:"A réaliser"},
  ], O.acc));
  ch.push(...sep());
  ...
  ch.push(...kpiRow([
    ...
    {label:"RMs sous-réal.", value:`${cal.rmsSPS.concat(cal.rmsCTC).filter(r=>r.decalage!=null&&r.decalage<-10).length}`, color:RED, accent:RED, sub:"à identifier"},
  ]));
  ch.push(note("Production nette = Réalisé – Avoirs   |   Décalage % = (Prod. nette – Prévu) / Prévu", GRAY2, GRAY3));
  ch.push(sp(80));

  if (cal.rmsSPS.length) {
    ch.push(p(r("Service SPS", {bold:true, size:18, color:BLUE}), {spacing:{before:80,after:60}}));
    ch.push(...dataTable(
      ["Chargé de mission","Prévu","Réalisé","Avoirs","Prod. nette","Décalage"],
      [
  ...
  ch.push(analyseBloc([
    "À compléter : commentaire global sur le calage du mois.",
    "À compléter : top performers à valoriser.",
    "À compléter : chargés de mission en retard — points d'attention.",
    "→ À compléter : décision ou action issue de l'analyse des calages.",
  ]));
  ch.push(...planActions([
    {action:"À compléter", qui:"À compléter", statut:"A réaliser"},
    {action:"À compléter", qui:"À compléter", statut:"A réaliser"},
  ...
  // 2.3 Prévisionnels N
  ch.push(...themeTitle("2.3  Prévisionnels N — 1er Janvier vs Réalisé Décembre", O.acc));
  const pN = d.previsionnels;
  ch.push(...kpiRow([
    {label:"Prévi. initiale",   value:fK(pN.agence.prev1janv), color:GRAY2, accent:GRAY2, sub:"au 1er janvier"},
    {label:"Réalisé",           value:fK(pN.agence.prevDec),   big:true, color:NAVY, accent:BLUE},
    {label:"Écart net",         value:fK(pN.agence.ecart),     color:pN.agence.ecart>=0?GREEN:RED, accent:BLUE},
    {label:"SPS écart",         value:fK(pN.sps.ecart),        color:BLUE, accent:BLUE},
    {label:"CTC écart",         value:fK(pN.ctc.ecart),        color:ORANGE, accent:ORANGE},
  ]));
  if (pN.rmsSPS.length) {
    ch.push(p(r("Service SPS", {bold:true, size:18, color:BLUE}), {spacing:{before:80,after:60}}));
    ch.push(...dataTable(
      ["Chargé de mission","Prévi. 1er Janv.","Réalisé Déc.","Écart"],
      [
  ...
  ch.push(analyseBloc([
    "À compléter : commentaire sur l'écart global prévi./réalisé.",
    "→ À compléter : leçons pour fiabiliser les prévisionnels N+1.",
  ]));
  ch.push(...planActions([{action:"À compléter", qui:"À compléter", statut:"A réaliser"}], O.acc));
  ch.push(PB());

  // ── 03 PRÉVISIONNELS N+1 ─────────────────────────────────────────
  ch.push(...sectionBanner("03","PRÉVISIONNELS N+1","",PR));
  ch.push(...themeTitle("3.1  Prévisionnels à fin N+1 par chargé de mission", PR.acc));
  const pNP1 = d.previsionnelsNP1 || d.previsionnels;
  ch.push(...kpiRow([
    {label:"Prév. Agence N+1", value:fK(pNP1.agence.prevDec), big:true, color:NAVY, accent:BLUE},
    {label:"SPS prévisionnel", value:fK(pNP1.sps.prevDec),    color:BLUE,   accent:BLUE},
    {label:"CTC prévisionnel", value:fK(pNP1.ctc.prevDec),    color:ORANGE, accent:ORANGE},
    {label:"Écart vs N",       value:fK((pNP1.agence.prevDec||0)-(prod.agence.total||0)),
      color:GRAY2, accent:GRAY2, sub:"par rapport au réalisé N"},
  ]));
  ch.push(note("Comparer le prévisionnel N+1 au réalisé N. Identifier les écarts significatifs et les leviers par RM.", BLUE, XBLUE));
  ch.push(sp(80));
  if ((pNP1.rmsSPS||[]).length) {
    ch.push(p(r("Service SPS", {bold:true, size:18, color:BLUE}), {spacing:{before:80,after:60}}));
    ch.push(...dataTable(
      ["Chargé de mission","Prév. fin N+1","Signal"],
      [
        [{value:"Agence SPS",bold:true}, fK(pNP1.sps.prevDec), "—"],
        ...(pNP1.rmsSPS||[]).map(rm=>[rm.rm, fK(rm.prevDec), rm.prevDec>0?'✓':'⚠ À compléter']),
      ],
  ...
  if ((pNP1.rmsCTC||[]).length) {
    ...
    ch.push(...dataTable(
      ...
      [
        ...
        ...(pNP1.rmsCTC||[]).map(rm=>[rm.rm, fK(rm.prevDec), rm.prevDec>0?'✓':'⚠ À compléter']),
      ],
      [3000,2000,2306], {compact:true, headerBg:PR.acc}
    ));
  }
  ch.push(analyseBloc([
    "À compléter : commentaire sur le niveau global des prévisionnels N+1 vs réalisé N.",
    "À compléter : chargés de mission avec prévisionnel faible ou vide.",
    "→ À compléter : date de validation des prévisionnels.",
  ]));
  ch.push(...planActions([
    {action:"Validation prévisionnels N+1 + 1 levier de dépassement par CM", qui:"Tous CMs", statut:"A réaliser"},
    {action:"À compléter : plan renforcé CMs en retard", qui:"RS + DA",   statut:"A réaliser"},
  ], PR.acc));
  ch.push(PB());

  // ── 04 AFFAIRES LT ───────────────────────────────────────────────
  ch.push(...sectionBanner("04","AFFAIRES DÉCALÉES LONG TERME","",AF));
  ch.push(...themeTitle("4.1  Volume décalé par service", AF.acc));
  const lt = d.lt;
  ch.push(...kpiRow([
  ...
  ch.push(...dataTable(
    ["Service","Volume LT","Part du total","Priorité"],
    [
  ...
  ch.push(note("Relier les affaires LT aux prévisionnels N+1 des CMs concernés. Identifier les top 3 à mobiliser en priorité.", AMBER, LAMB));
  ch.push(analyseBloc([
    "À compléter : commentaire sur la répartition des affaires décalées (SPS/CTC).",
    "À compléter : leviers de mobilisation identifiés.",
    "→ À compléter : top 3 affaires prioritaires à débloquer.",
  ]));
  ch.push(...planActions([
    {action:"Top 3 affaires LT à mobiliser — SPS (plan daté)", qui:"RS SPS", statut:"A réaliser"},
    {action:"Top 3 affaires LT à mobiliser — CTC (plan daté)", qui:"RS CTC", statut:"A réaliser"},
  ], AF.acc));
  ch.push(PB());
  ...
  ch.push(...dataTable(
    ["Sujet","Détail","Statut"],
    [
      ["Arrivées prévues","À compléter","—"],
      ["Postes à pourvoir","À compléter","—"],
      ["Départs","À compléter","—"],
      ["Besoins formation","À compléter avant le 15 du mois","—"],
      ["Organisation locale","À compléter","—"],
    ],
    [2200,5010,3096], {headerBg:RH.acc}
  ));
  ch.push(analyseBloc(["À compléter : mouvements du mois, besoins identifiés, organisation locale."]));
  ch.push(...planActions([
    {action:"Point RH préparé (5 min max)",               qui:"DA",      statut:"A réaliser"},
    {action:"Besoins formation remontés avant le 15",      qui:"DA + RS", statut:"A réaliser"},
  ], RH.acc));
  ch.push(...sep());

  ch.push(...themeTitle("5.2  Sujets opérationnels & Vie d'agence", RH.acc));
  ch.push(...dataTable(
    ["Thème","Contenu","Attendu"],
    [
      [{value:"Sécurité",     bold:true},"EPI · retours terrain · incidents",               {value:"Partage équipe",  color:RED}],
      [{value:"Qualité",       bold:true},"Nouvelles maquettes · notes · procédures",         {value:"Diffusion",       color:GREEN}],
      [{value:"Communication", bold:true},"Descente CODIR · alertes · signaux faibles",       {value:"DA -> équipe",    color:BLUE}],
      [{value:"Événements",    bold:true},"Audits · visites médicales · congés",              "Planification"],
      [{value:"Projets DT",    bold:true},"Nouveaux outils · process supports · moyens",     {value:"Présentation",    color:AMBER}],
    ],
    [1800,5010,1496+500-500], {headerBg:RH.acc}
  ));
  ch.push(analyseBloc([
    "La sécurité est le seul point non-négociable à chaque réunion.",
    "→ Remontée des alertes et signaux faibles vers la direction.",
  ]));
  ch.push(...planActions([
    {action:"Point sécurité mensuel obligatoire",         qui:"DA",      statut:"En cours"},
    {action:"Diffusion nouveautés DT et outils aux CMs",  qui:"DA + RS", statut:"A réaliser"},
  ], RH.acc));
  ch.push(PB());

  // ── 06 SYNTHÈSE ───────────────────────────────────────────────────
  ch.push(...sectionBanner("06","SYNTHÈSE — PLAN D'ACTIONS","",SY));
  ...
  const hRow = new TableRow({ children:[
    ...
    tc(p(r("Échéance",     {bold:true,size:16,color:WHITE}),{align:AlignmentType.CENTER}), {w:WPE,bg:NAVY,brd:bNone()}),
  ...
  const sections = [
    {section:"Commerce"},
    {action:"[À compléter]",qui:"",ech:"",statut:"A réaliser"},
    {action:"[À compléter]",qui:"",ech:"",statut:"A réaliser"},
    {section:"Opérations"},
    {action:"[À compléter]",qui:"",ech:"",statut:"A réaliser"},
    {action:"[À compléter]",qui:"",ech:"",statut:"En cours"},
    {section:"Prévisionnels N+1"},
    {action:"Validation prévisionnels N+1 + 1 levier par CM",qui:"Tous CMs",ech:"À définir",statut:"A réaliser"},
    {section:"Affaires LT"},
    {action:"Top 3 affaires LT SPS",qui:"RS SPS",ech:"À définir",statut:"A réaliser"},
    {action:"Top 3 affaires LT CTC",qui:"RS CTC",ech:"À définir",statut:"A réaliser"},
    {section:"RH & Vie d'agence"},
    {action:"Point RH préparé (5 min)",    qui:"DA",     ech:"Chaque réunion",statut:"A réaliser"},
    {action:"Point sécurité mensuel",      qui:"DA",     ech:"Chaque réunion",statut:"En cours"},
    {action:"[À compléter]",qui:"",ech:"",statut:"A réaliser"},
  ];
  let gi=0;
  ...
  ch.push(tbl([W1,W2], [new TableRow({ children:[
    tc([
      ...
      p(r("Date & lieu : À définir  ·  OdJ à transmettre sous 8 jours",{size:17,color:GRAY2})),
...
// logoData = { data: Uint8Array, type: 'png'|'jpeg' } ou null
async function buildCR(entity, moisLabel, logoData=null) {
